...
window.addEventListener('DOMContentLoaded',()=>{
  const h=new Date().getHours(); const isDay=h>=5&&h<19;
  if(isDay){setTheme('classic',true);} else {setTheme('heritage',false);}
  setTimeout(()=>{if(userManuallyChangedTheme)return; introPhase=false; if(isDay){setTheme(themeByTime(),false);} else {setTheme(themeByTime(),true);}},30000);
  setInterval(()=>{if(!userManuallyChangedTheme&&!introPhase){const night=isNightNow(); setTheme(themeByTime(),night?true:false);}},3600000);
